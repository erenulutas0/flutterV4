...
class WebRTCManager {
  ...
  setupLocalVideo() {
    ...
    if (!container) {
      ...
      container.style.zIndex = '10000'; // En üstte olmalı (kontrollerin üzerinde)
  ...
  setupRemoteVideo(stream) {
    ...
    if (!container) {
      ...
      container.style.zIndex = '0'; // Flutter Stack'in altında, en altta
      container.style.backgroundColor = '#000';
      container.style.pointerEvents = 'none'; // Tıklamaları geçir
      // Body'ye direkt ekle (Flutter Stack'in altında olacak)
      document.body.insertBefore(container, document.body.firstChild); // En başa ekle
      console.log('✅ Created remote video container with z-index 0 (below Flutter Stack) - v7 - CACHE FIX');
    }
        ...
        try {
          ...
          if (!this.peerConnection) {
            ...
            if (!this.localStream) {
              ...
              // getUserMedia henüz tamamlanmamış, bekleyelim
              try {
